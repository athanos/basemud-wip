...
void dequeue_event(EVENT_DATA *event)
{
  ...
  switch(event->ownertype)
  {
    ...
    case EVENT_OWNER_CHAR:
      ...
      break;
    case EVENT_OWNER_SOCKET:
      DetachFromList(event, event->owner.sock->events);
      break;
  }
...
void add_event_socket(EVENT_DATA *event, SOCKET_DATA *sock, int delay)
{
  ...
  /* set the correct variables for this event */
  event->ownertype   = EVENT_OWNER_SOCKET;
  event->owner.sock = sock;

  /* attach the event to the sockets local list */
  AttachToList(event, sock->events);
...
EVENT_DATA *event_isset_socket(SOCKET_DATA *sock, int type)
{
  ...
  AttachIterator(&Iter, sock->events);
  while ((event = (EVENT_DATA *) NextInList(&Iter)) != NULL)
  {
    if (event->type == type)
      break;
...
void init_events_socket(SOCKET_DATA *sock)
{
  ...
  add_event_socket(event, sock, 5 * 60 * PULSES_PER_SECOND);
}
